...
import { Star, Heart, ShoppingBag, Truck, Shield, RotateCcw, Check, Minus, Plus } from 'lucide-react';
        ...
        <div className="grid lg:grid-cols-2 gap-12 items-start">
          ...
          <div className="space-y-4">
            ...
            <div className="relative aspect-square rounded-3xl overflow-hidden bg-muted/30 shadow-2xl shadow-primary/10">
              ...
              />
              {discount > 0 && (
                <span className="absolute top-4 left-4 px-4 py-2 bg-primary text-primary-foreground font-semibold rounded-full">
                  -{discount}% OFF
                </span>
              )}
            </div>
              ...
              <div className="flex gap-3 justify-center">
                {product.images.map((image, index) => (
                  <button
                    ...
                    className={cn(
                      "relative w-20 h-20 rounded-xl overflow-hidden transition-all duration-300",
                      selectedImage === index 
                        ? "ring-2 ring-primary ring-offset-2 scale-105" 
                        : "ring-1 ring-border/50 hover:ring-primary/50 opacity-70 hover:opacity-100"
                    )}
                  >
                    <Image
                      src={image}
          ...
          <div className="lg:sticky lg:top-24 space-y-6">
            ...
            <div className="flex gap-4">
              <button 
                className={cn(
                  "flex-1 flex items-center justify-center gap-3 px-8 py-4 rounded-2xl font-semibold text-lg transition-all duration-300",
            ...
            <div className="grid grid-cols-3 gap-4 pt-6">
              <div className="flex flex-col items-center gap-2 p-4 rounded-xl bg-muted/30">
                ...
                <span className="text-xs text-center text-muted-foreground">Envio gratis +$150.000</span>
              </div>
              <div className="flex flex-col items-center gap-2 p-4 rounded-xl bg-muted/30">
                <Shield className="w-6 h-6 text-primary" />
                <span className="text-xs text-center text-muted-foreground">Compra segura</span>
              </div>
              <div className="flex flex-col items-center gap-2 p-4 rounded-xl bg-muted/30">
                <RotateCcw className="w-6 h-6 text-primary" />
                <span className="text-xs text-center text-muted-foreground">30 dias devolucion</span>
              </div>
            </div>
          </div>
        </div>
